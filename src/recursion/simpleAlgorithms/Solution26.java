...
public class Solution26 {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            generatePaths(writer, "", depth, K, -1);
            writer.close();
    ...
    public static void generatePaths(FileWriter writer, String path, int depth, int K, int lastNode) throws IOException {
        if (depth == 0) {
            writer.write(path + "\n");
            return;
        }
        ...
        for (int i = 1; i <= K; i++) {
            if (i != lastNode) {
                generatePaths(writer, path + i, depth - 1, K, i);
            }
        }
    }
